...
const NewsArticle = ({ article, bookmarkMsgHandler }) => {
    ...
    // Get the correct image URL regardless of source
    const getImageUrl = () => {
    ...
    return (
        <div className={`news-article ${isMobileDevice && "mobile-news-article"}`} onClick={articleHandler} style={{ height: isMobileDevice && windowHeight }}>
            ...
            <div className="content">
                ...
                <span className="author-time"><b>short</b> by {article.source?.name || 'Unknown Source'} / {`${hours}:${minutes} ${meridiem} on ${day}, ${date} ${month}, ${year}`}</span>
                <p className="description">{article.description}</p>
                <span className="source">read more at <a href={article.url} target="_blank" className="name">{article.source?.name || 'Source'}</a></span>
            </div>
            ...
            {isMobileDevice && <div className="bottom-section">
                ...
                <section>
                    <span>To see the full image</span><br />
                    <a href={getImageUrl()} target="_blank" className="image-link">Tap here</a>
                </section>
